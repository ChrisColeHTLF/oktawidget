...
(function configureEvergage() {
    try {
       Evergage.configure({
           account: "heartlandfinancial",
           dataset: "prod",
           cdnUrl: "https://cdn.evergage.com",
           trackerUrl: "https://heartlandfinancial.us-4.evergage.com",
           siteConfigVersion: "34"
       },{"allowBotTraffic":false,"corsAllowedOrigins":["*"],"defaultCurrency":"USD","defaultLocale":null,"disableQtipWindowScroll":false,"doNotStoreCookiesRequireProvidedAnonId":false,"doNotTrackPingRequestsForActions":false,"enableCorsRestrictedOrigins":false,"enableMessageRotation":false,"enableRememberMeUserIds":false,"enableTemplateEsc":false,"hideContentSections":true,"hideContentSectionsMillis":2500,"hidePagesForRedirect":false,"hidePagesForRedirectMillis":1000,"identityAttributes":["profileId"],"lastModified":1623102015827,"preventSensitiveDataCapture":false,"rememberMeUserIdsMillis":63072000000,"sendErrorEvents":true,"showPoweredBy":false,"siteConfigExecBeforePageReady":false,"spaRouteChangeTimeout":500,"trackAnonymousVisitors":true,"trackContextualRelatedItems":false,"trackSubdomainAsCompany":false,"trackUnknownPagesByTitle":false,"treatHashChangeAsPageLoad":false}       );
    } catch (e) {
          console.error(e);
    }
})();
...
const getCookieDomain = () => {
    let domains =
        ['www.dubuquebank.com',
            ...
            'www.firstbanktexas.com',
            'www.heartlandrps.com'];
...
Evergage.init({
    ...
    //map for banks to get bankname and shortname
    let bankMap = new Map([
        ['www.dubuquebank.com', { name: 'Dubuque Bank and Trust', shortName: 'dbt' }],
        ['www.illinoisbank.com', { name: 'Illinois Bank & Trust', shortName: 'ibt' }],
        ['www.wisconsinbankandtrust.com', { name: 'Wisconsin Bank & Trust', shortName: 'wbt' }],
        ['www.nmb-t.com', { name: 'New Mexico Bank & Trust', shortName: 'nmbt' }],
        ['www.arizbank.com', { name: 'Arizona Bank & Trust', shortName: 'abt' }],
        ['www.rmbank.com', { name: 'Rocky Mountain Bank', shortName: 'rmb' }],
        ['www.citywidebanks.com', { name: 'Citywide Banks', shortName: 'cwb' }],
        ['www.mnbankandtrust.com', { name: 'Minnesota Bank & Trust', shortName: 'mbt' }],
        ['www.bankbv.com', { name: 'Bank of Blue Valley', shortName: 'bbv' }],
        ['www.premiervalleybank.com', { name: 'Premier Valley Bank', shortName: 'pvb' }],
        ['www.firstbanktexas.com', { name: 'FirstBank & Trust', shortName: 'fbt' }],
        ['www.heartlandrps.com', { name: 'Retirement Plan Services', shortName: 'rps' }]
    ]);
    const productDimensionClickText = 'See More';
    //get the bankname and shortname from the bank map
    let bankName = bankMap.has(window.location.hostname) ? bankMap.get(window.location.hostname).name : '';
    let bankShortName = bankMap.has(window.location.hostname) ? bankMap.get(window.location.hostname).shortName : '';

    const canAccessResource = () => !/\b40[34]\b/.test(Evergage.cashDom("link[rel=canonical]").attr("href"));
    ...
    const sitemapConfig = {
        global: {
            ...
            onActionEvent: (actionEvent) => {
                try {
                    if ((bankName && bankShortName) || window.location.search.indexOf('evgTestUser') > -1) {
                        actionEvent.user = actionEvent.user || {};
                        actionEvent.user.attributes = actionEvent.user.attributes || {};
                        if (bankName && bankShortName){
                            actionEvent.user.attributes.bankName = bankName;
                            actionEvent.user.attributes.bankShortName = bankShortName;
                        }
                        if(window.location.search.indexOf('evgTestUser') > -1){
            ...
            listeners: [
                //Login
                Evergage.listener('submit', 'form[name="Q2OnlineBanking"]', () => {
                    let userId = Evergage.cashDom('#userID').val();
                    if (userId) {
                        sendEvent('Log-in', userId);
                    }
                }),
                //Mobile Login
                Evergage.listener('submit', 'form[name="Q2OnlineLogin"]', () => {
                    let userId = Evergage.cashDom('#mobile-username').val();
                    if (userId) {
                        sendEvent('Mobile Log-in', userId);
                    }
                }),
                //Enroll
                Evergage.listener('click', 'form[name="Q2OnlineBanking"] .login-links ul', (e) => {
                    let label = String(e.target.innerText);
                    label = label.trim();
                    if (label.includes('Enroll')) {
                        sendEvent('Enroll', '');
                    }
                }),
                //Open Account
                Evergage.listener('click', '.account-links a.button.margin-right', (e) => {
                    sendEvent('Open Account ', '');
                }),
                //Apply Now
                Evergage.listener('click', '[aria-label$="CASH BACK REWARDS CARD"]', (e) => {
                    sendEvent('Apply Now', '');
                }),
                //Find a Banker
                Evergage.listener('click', '.region  a.button', (e) => {
                    let label = String(e.target.innerText);
                    label = label.trim();
                    if (label.includes('Find a Banker')) {
                        sendEvent('Find a Banker', '');
                    }
                }),
                //Enroll Now
                Evergage.listener('click', '.catergory-links a.button', (e) => {
                    let label = String(e.target.innerText);
                    label = label.trim();
                    if (label.includes('Enroll Now')) {
                        let h2TextId = e.target.parentNode.children[0].getAttribute('aria-labelledby');
                        let h2Text = Evergage.cashDom('#' + h2TextId).text().trim();
                        sendEvent(h2Text + ' Enroll Now', '');
                    }
                }),
            ],
        },
        ...
        pageTypes: [
            {
                //homepage
                name: "home",
                action: "Homepage - " + bankName,
                isMatch: () => Evergage.cashDom('link[rel="canonical"][href*="homepage"').length > 0 && /^\/$/.test(window.location.pathname),
                contentZones: [
                    {
                        name: "hero_zone_1", selector: ".evg-hero-block-1"
                    },
                    {
                        name: "hero_zone_2", selector: ".evg-hero-block-2"
                    },
                    {
                        name: "offer_zone_1", selector: ".evg-" + bankShortName + "-home-ob-1"
                    },
                    {
                        name: "offer_zone_2", selector: ".evg-" + bankShortName + "-home-ob-2"
                    },
                ]
            },
            {
                //Personal page
                name: "personal",
                action: ["Personal Banking", Evergage.util.extractFirstGroup(/^(.*?) \W/, Evergage.cashDom("title").text())].join(" | "),
                isMatch: () => /\/(personal-banking|anytimebanking)/.test(window.location.pathname) && Evergage.DisplayUtils.pageElementLoaded("H1", "html").then(() => true),
                catalog: {
                    Product: {
                        _id: Evergage.resolvers.fromHref((elm)=>{ let productName= new URL(elm); return productName.protocol + '//' + productName.hostname + productName.pathname}),
                        name: Evergage.resolvers.fromSelector('H1', (elm) => elm.trim()),
                        url: Evergage.resolvers.fromHref((elm) => Evergage.util.removeQueryString(elm)),
                        description: Evergage.resolvers.fromSelectorAttribute('meta[name="description"]', 'content', (desc) => desc ? desc.trim(): ''),
                        categories: () => [["Personal Banking", Evergage.util.extractFirstGroup(/^(.*?) \W/, Evergage.cashDom("title").text())].join("|")],
                        dimensions: {
                            SegmentOfInterest: () => ['Personal Banking'],
                            BankName: () => [bankName],
                            Style: () => ['Personal Banking'],
                            Brand: () => [bankName],
                            ItemClass: () => [Evergage.resolvers.fromSelector('H1', (elm) => elm.trim())]
                        }
                    }
                },
                listeners: [
                    //Regular Savings - Open Account
                    Evergage.listener('click', '.catergory-links a.button', (e) => {
                        let label = String(e.target.innerText);
                        label = label.trim();
                        if (label.includes('Open Account')) {
                            let h2TextId = e.target.parentNode.children[0].getAttribute('aria-labelledby');
                            let h2Text = Evergage.cashDom('#' + h2TextId).text().trim();
                            sendEvent(h2Text + ' - Open Account', '');
                            Evergage.sendEvent({
                                action: h2Text + ' - Open Account',
                                itemAction: Evergage.ItemAction.ViewItemDetail,
                                catalog: {
                                    Product: {
                                        _id: new URL(window.location.href).protocol + '//' + new URL(window.location.href).hostname + new URL(window.location.href).pathname,
                                        dimensions: {
                                            ProductType: [h2Text]
                                        }
                                    }
            ...
            },
            {
                //Small Business Page
                name: "small_business",
                action: ["Small Business Banking", Evergage.util.extractFirstGroup(/^(.*?) \W/, Evergage.cashDom("title").text())].join(" | "),
                isMatch: () => /\/small-business-banking/.test(window.location.pathname) && Evergage.DisplayUtils.pageElementLoaded("H1", "html").then(() => true),
                catalog: {
                    Product: {
                        _id: Evergage.resolvers.fromHref((elm)=>{ let productName= new URL(elm); return productName.protocol + '//' + productName.hostname + productName.pathname}),
                        name: Evergage.resolvers.fromSelector('H1', (elm) => elm.trim()),
                        url: Evergage.resolvers.fromHref((elm) => Evergage.util.removeQueryString(elm)),
                        description: Evergage.resolvers.fromSelectorAttribute('meta[name="description"]', 'content', (desc) => desc ? desc.trim(): ''),
                        categories: () => [["Small Business Banking", Evergage.util.extractFirstGroup(/^(.*?) \W/, Evergage.cashDom("title").text())].join("|")],
                        dimensions: {
                            SegmentOfInterest: () => ['Small Business'],
            ...
            },
            {
                //Commercial page
                name: "commercial",
                action: ["Commercial Banking", Evergage.util.extractFirstGroup(/^(.*?) \W/, Evergage.cashDom("title").text())].join(" | "),
                isMatch: () => /\/(commercial-banking|retirement-plan-services)/.test(window.location.pathname) && Evergage.DisplayUtils.pageElementLoaded("H1", "html").then(() => true),
                catalog: {
                    Product: {
                        _id: Evergage.resolvers.fromHref((elm)=>{ let productName= new URL(elm); return productName.protocol + '//' + productName.hostname + productName.pathname}),
                        name: Evergage.resolvers.fromSelector('H1', (elm) => elm.trim()),
                        url: Evergage.resolvers.fromHref((elm) => Evergage.util.removeQueryString(elm)),
                        description: Evergage.resolvers.fromSelectorAttribute('meta[name="description"]', 'content', (desc) => desc ? desc.trim(): ''),
                        categories: () => [["Commercial Banking", Evergage.util.extractFirstGroup(/^(.*?) \W/, Evergage.cashDom("title").text())].join("|")],
                        dimensions: {
                            SegmentOfInterest: () => ['Commercial Banking'],
                            Style: () => ['Commercial Banking'],
                            Brand: () => [bankName],
                            ItemClass: () => [Evergage.resolvers.fromSelector('H1', (elm) => elm.trim())]
                        }
                    }
            ...
            },
            {
                ...
                isMatch: () => /\/business-insights/.test(window.location.pathname),
            },
            {
                //News Page
                name: "news",
            ...
            },
            {
                ...
                name: "stories",
                action: "Stories",
                isMatch: () => /\/stories/.test(window.location.pathname),
            },
            {
                //Locations
                name: "locations",
                action: "Locations",
                isMatch: () => window.location.pathname === '/locations',
            }
        ]
    }
    const sendEvent = (eventName, loginId) => {

        //if login is not blank, send login Id in user attributes otherwise, just send bankname and shortname
        Evergage.sendEvent({
            action: eventName,
            user: {
                ...(loginId && bankShortName && { id: bankShortName + '_' + loginId }),
                attributes: {
                    ...(loginId && { loginId: bankShortName + '_' + loginId }),
                    ...(bankName && { bankName: bankName }),
                    ...(bankShortName && { bankShortName: bankShortName })
    ...
    window.addEventListener("DOMContentLoaded", () => {
        let frame = document.querySelector("iframe[title='Commercial Contact']");
        if (!frame && frame.contentDocument) return;

        frame.addEventListener("load", () => {
            let form = frame.contentDocument.querySelector(".jotform-form");
            form.addEventListener("submit", (e) => {
                let $target = Evergage.cashDom(e.target);
                let customerName = $target.find("#input_11").not(".form-validation-error").val();
                let areaCode = $target.find("#input_8_area").not(".form-validation-error").val(); 
                let phoneNumber = $target.find("#input_8_phone").not(".form-validation-error").val(); 
                let workPhone;
                if (areaCode && phoneNumber) {
                    workPhone = areaCode + phoneNumber;
                }
                let emailAddress = $target.find("#input_5").not(".form-validation-error").val();
                let userAttrs = {
                ...
                if (!Object.values(userAttrs)?.length > 0) return;

                Evergage.sendEvent({
                    action: "Contact a Banker",
                    user: {
    ...
    if (currentCookieDomain && canAccessResource()) {
